...
from django_sso.sso_service import get_sso_authorization_request, request_sso_authorization_request, \
    request_deauthentication, set_sso_authorization_request_used
...
def event_acceptor_view(request):
    ...
    #
    print('!!!!'*10, 'write user to session', data.get('username'))
    request.session['sso_user'] = data.get('username')
